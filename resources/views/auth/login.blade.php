<x-guest-layout>
    ...
    <div class="mb-6 text-center border-b-2 border-blue-100 pb-4">
        ...
        <p class="text-sm text-gray-500 mt-1">Gunakan NIM, NIP, atau Nama PT sebagai Username.</p>
    </div>
    ...
    <form method="POST" action="{{ route('login') }}">
        ...
        <!-- Input Tersembunyi: Mengirim parameter role ke Controller -->
        <input type="hidden" name="expected_role" value="{{ request('role') }}">

        <!-- Username (NIM / NIP / PT) -->
        <div>
            <x-input-label for="username" :value="__('Username (NIM/NIP/PT)')" />
            <x-text-input id="username" class="block mt-1 w-full border-2 border-blue-200 focus:border-blue-600 focus:ring focus:ring-blue-200 focus:ring-opacity-50 rounded-md shadow-sm font-medium" type="text" name="username" :value="old('username')" required autofocus autocomplete="username" />
